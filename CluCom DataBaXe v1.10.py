...
def Login():
        ...
        if user.get()==2:
                ...
                conn.commit()
                conn.close()
                #print(staff_lst_fetch) # <-- LIFE SAVER!!!!
                ...
                if len(staff_lst_fetch)==0:
                       ...
                       med_tab = ttk.Notebook(med_win,bootstyle=DARK)
                       ...
                       doc_label = ttk.Label(my_frame31,text="Welcome Dr.",style="medlabels.TLabel",font=("Arial","16")).grid(row=0,column=1,columnspan=2)
                       for docs in staff_lst_fetch:
                               print(docs)
                               if docs[0][1]==e.get():
                                       doc_name_label = ttk.Label(my_frame32,text=docs[0][4],style="medlabels.TLabel",font=("Arial","16")).grid(row=0,column=1,columnspan=2)
                               else:
                                       pass
                       ...
                       for row in entries3:
                              #inserts rows from db to treeview ""-->(start) to tk.END--> values=rows of db one by one
                              tree.insert("", "end", values=row)
                       tree.grid(row=4,column=1,columnspan=5,sticky="nsew")
...
#
                      ...
                      #def choose_dept():
                      #        selected_value.get()
                      #        return #RETURN TO APTT DB
                      #select_dept = ttk.Button(my_frame3,text="OK",command=choose_dept,bootstyle=PRIMARY).grid(row=3,column=3)
