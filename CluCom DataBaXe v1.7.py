...
def Login():
        ...
        if user.get()==3:
              ...
              if len(user_fetch)==0:
                      ...
                      def show():
                                ...
                                tree = ttk.Treeview(new_win,columns=col_names,show="headings",bootstyle=PRIMARY)
                      ...
                      #def alter():
                      #        alter_label = ttk.Label(new_win,text="Enter new column name>")
                      #        alter_label.grid(column=1,row=0)
                      #        col_name2 = ttk.Entry(new_win,width=30)
                      #        col_name2.grid(column=2,row=0)
                      #        conn = sqlite3.connect('OPD2.db')
                      #        c.execute("ALTER TABLE OPD2 ADD COLUMN = ? BLOB")
                      #button_alter = ttk.Button(new_win,text="Alter Table",command=alter)
        ...
        if user.get()==1:
              ...
              if len(user_fetch)==0:
                      ...
                      my_tab.add(my_frame4,text="Settings")
                      my_tab.add(my_frame5,text="Log Out"  )
                      ...
                      # BELOW PAGES TO BE DESIGNED LATER (modify record, delete records, upload pdf, upload images to REC.db)
                      def logout():
                              pat_dash.destroy()
