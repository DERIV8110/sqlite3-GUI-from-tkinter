...
def sign_up_patient():
        ...
        def submit():
               ...
               if user_check is None:
                      ...
                      c.execute("""INSERT INTO OPD2(Photo,Name,Username,Password,Age,Gender,Address,Phone,Email,Medical,Doctor,Department,Diagnosis) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                 (blob,name_entry.get(),username_entry.get(),password_entry.get(),age_mnubtn.cget("text"),gender_var.get(),address_entry.get(),phone_entry.get(),email_entry.get(),medical_file_entry.get(),NONE,NONE,NONE))
        ...
        age_mnubtn = ttk.Menubutton(signup_win, width=27, text="Select Age")
        age_mnubtn.grid(row=5, column=2)
        age_menu = Menu(age_mnubtn, tearoff=0)
        age_mnubtn["menu"] = age_menu
        for age in range(1, 99):
                age_menu.add_radiobutton(label=str(age), command=lambda a=age: age_mnubtn.configure(text=str(a)))

        #age_entry=ttk.Entry(signup_win,width=30)
        #age_entry.grid(row=5,column=2)
        gender_var=StringVar(value="Select Gender")
        
        male_rb =   ttk.Radiobutton(signup_win, text="Male", variable=gender_var, value="Male", bootstyle=PRIMARY)
        female_rb = ttk.Radiobutton(signup_win, text="Female", variable=gender_var, value="Female", bootstyle=PRIMARY)
        other_rb =  ttk.Radiobutton(signup_win, text="Other", variable=gender_var, value="Other", bootstyle=PRIMARY)
        male_rb.grid(row=6,   column=2, padx=5)
        female_rb.grid(row=6, column=3, padx=5)
        other_rb.grid(row=6,  column=4, padx=5)      
        ...
        name_label           =ttk.Label(signup_win,    text='Name',font=("Segoe UI","10")).grid(row=2,column=1,    sticky="nsew")
        username_label       =ttk.Label(signup_win,    text='Username',font=("Segoe UI","10")).grid(row=3,column=1,sticky="nsew") 
        password_label       =ttk.Label(signup_win,    text='Password',font=("Segoe UI","10")).grid(row=4,column=1,sticky="nsew")
        age_label            =ttk.Label(signup_win,    text='Age',font=("Segoe UI","10")).grid(row=5,column=1,     sticky="nsew")
        gender_label         =ttk.Label(signup_win,    text='Gender',font=("Segoe UI","10")).grid(row=6,column=1,  sticky="nsew")
        address_label        =ttk.Label(signup_win,    text='Address',font=("Segoe UI","10")).grid(row=7,column=1, sticky="nsew")
        phone_label          =ttk.Label(signup_win,    text='Phone',font=("Segoe UI","10")).grid(row=8,column=1,   sticky="nsew")
        email_label          =ttk.Label(signup_win,    text='Email ID',font=("Segoe UI","10")).grid(row=9,column=1,sticky="nsew")
        medical_history_label=ttk.Label(signup_win,    text='Enter/Upload History',font=("Segoe UI","10")).grid(sticky="w", row=10,column=1)
        submit_button=        ttk.Button(signup_win,text="Submit",        command=submit).grid(row=11,column=3,ipadx=10,ipady=5)
...
def sign_up_staff():
        ...
        def verify_auth_code():
                ...
                if auth_entry.get()=="1234":
                        ...
                        submit_button=        ttk.Button(signup_win,text="Submit",        command=submit,bootstyle=DANGER).grid(row=11,column=3,ipadx=10,ipady=5)
                else:
...
def Login():
        ...
        if user.get()==3:
              ...
              if len(user_fetch)==0:
                      ...
                      my_tab2 = ttk.Notebook(new_win,bootstyle=DANGER)
                      ...
                      my_tab2.add(my_frame11,text="Patients'")
                      my_tab2.add(my_frame21,text="Staffs'"  )
                      ...
                      # INSTEAD OF TWO
                                ...
                                # FOR STAFF DB
                                tree = ttk.Treeview(my_frame21,columns=col_names2,show="headings",bootstyle=DANGER)
                                ...
                                for row in entries2:
                                       ...
                                       tree.insert("", "end", values=row)
                                tree.grid(row=4,column=2,columnspan=5,sticky="nsew")
                      button_show_all= ttk.Button(new_win,text="Refresh",command=show,bootstyle=DANGER)
                      button_show_all.grid(column=0,row=1,columnspan=2)
                      def del_user_id():
                                  def delete():
                                        conn= sqlite3.connect('OPD2.db')
                                        c= conn.cursor()
                                        c.execute("DELETE FROM OPD2 WHERE ID = ?",(del_id_entry.get(),))
                                        conn.commit()
                                        conn.close()
                                  ...
                                  del_frame = ttk.Frame(del_win)
                                  del_win.grid_rowconfigure(0, weight=1)
                                  del_win.grid_columnconfigure(0, weight=1)
                                  del_frame.grid(row=0,column=0)
                                  del_label = ttk.Label(del_win,text="Enter User ID to Delete:")
                                  del_label.grid(row=0,column=0,columnspan=2)
                                  del_id_entry = ttk.Entry(del_win,width=30)
                                  del_id_entry.grid(row=1,column=0,columnspan=2)
                                  del_button = ttk.Button(del_win,text="Delete",command=delete,bootstyle=DANGER)
                                  del_button.grid(row=2,column=0,columnspan=2)
                      del_user = ttk.Button(new_win,text="Delete User",command=del_user_id,bootstyle=DANGER)
                      del_user.grid(column=0,row=2,columnspan=2)
        ...
        if user.get()==1:
              ...
              if len(user_fetch)==0:
                      ...
                      #Notebook function creates navigation pages/tabs based on same frame but different page
                      my_tab = ttk.Notebook(pat_dash,bootstyle=PRIMARY)
                      ...
                      my_frame4.grid_rowconfigure(0, weight=2)
                      my_frame4.grid_rowconfigure(1, weight=2)
                      my_frame4.grid_rowconfigure(2, weight=2)
                      my_frame4.grid_rowconfigure(3, weight=2)
                      my_frame4.grid_rowconfigure(4, weight=2)
                      my_frame4.grid_rowconfigure(5, weight=2)
                      my_frame4.grid_rowconfigure(6, weight=2)
                      my_frame4.grid_rowconfigure(7, weight=2)

                      my_frame4.grid_columnconfigure(0, weight=2)
                      my_frame4.grid_columnconfigure(1, weight=2)
                      my_frame4.grid_columnconfigure(2, weight=2)
                      my_frame4.grid_columnconfigure(3, weight=2)
                      my_frame4.grid_columnconfigure(4, weight=2)
                      my_frame4.grid_columnconfigure(5, weight=2)
                      ...
                      diagnosisdoc_button =ttk.Button(my_frame2,text="Open in Browser",bootstyle=PRIMARY,command=open_file).grid(row=2,column=2)
                      if len(doc_out)==0:
                      ...
                      #UPDATE DETAILS PAGE
                      def update_photo():
                                    file_path = filedialog.askopenfilename(title="Select Photo",filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.bmp;*.gif")])
                                    if file_path:
                                            with open(file_path, 'rb') as file:
                                                    photo_data = file.read()
                                            conn= sqlite3.connect('OPD2.db')
                                            c= conn.cursor()
                                            c.execute("UPDATE OPD2 SET Photo = ? WHERE Username = ?",(photo_data,e.get()))
                                            conn.commit()
                                            conn.close()
                                            messagebox.showinfo("Update Photo","Photo updated successfully!\n Restart app to see changes.")
                      update_photo_label = ttk.Label(my_frame4,text="Change Photo(100px100p)*",font=("Segoe UI", 18)).grid(row=1,column=0)
                      up_photo_btn = ttk.Button(my_frame4,text="Upload",bootstyle=PRIMARY,command=update_photo).grid(column=2,row=1,ipadx=7)
                      
                      
                      dept_menu = ttk.Menubutton(my_frame4, text="Select")
                      selected_value = ttk.StringVar(value="None")
                      menu = Menu(dept_menu,tearoff=0)
                      dept_menu.config(menu=menu)
                      for option in ["Orthopaedics","Opthalmology","General medicine","Gastroenterology"]:
                      ...
                      def update_dept():
                                    def update_dept_db():
                                            conn= sqlite3.connect('OPD2.db')
                                            c= conn.cursor()
                                            c.execute("UPDATE OPD2 SET Department = ? WHERE Username = ?",(selected_value.get(),e.get()))
                                            conn.commit()
                                            conn.close()
                                            messagebox.showinfo("Update Dept","Department updated successfully!\n Restart app to see changes.")
                                    update_dept_confirm_btn = ttk.Button(my_frame4,text="Update",bootstyle=DANGER,command=update_dept_db).grid(column=2,row=2,ipadx=7)
                      update_dept_label = ttk.Label(my_frame4,text="Update Department",font=("Segoe UI", 18)).grid(row=2,column=0)
                      update_dept_btn = ttk.Button(my_frame4,text="Update Department",bootstyle=PRIMARY,command=update_dept).grid(column=3,row=2,ipadx=7)
                      def update_diag_docs():
                                    file_path = filedialog.askopenfilename(title="Select Diagnosis Document",filetypes=[("PDF Files", "*.pdf")])
                                    if file_path:
                                            with open(file_path, 'rb') as file:
                                                    diag_doc_data = file.read()
                                            conn= sqlite3.connect('OPD2.db')
                                            c= conn.cursor()
                                            c.execute("UPDATE REC SET DiagnosisDoc = ? WHERE Username = ?",(diag_doc_data,e.get()))
                                            conn.commit()
                                            conn.close()
                                            messagebox.showinfo("Update Diagnosis Docs","Diagnosis Documents updated successfully!\n Restart app to see changes.")
                      update_diag_docs_label = ttk.Label(my_frame4,text="Upload Diagnosis Documents",font=("Segoe UI", 18)).grid(row=3,column=0,columnspan=2)
                      update_diag_docs_btn = ttk.Button(my_frame4,text="Upload File",bootstyle=PRIMARY,command=update_diag_docs).grid(column=2,row=3,ipadx=7)
                      def logout():
                              pat_dash.destroy()
