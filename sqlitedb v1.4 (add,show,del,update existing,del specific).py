...
root.title("Vapid DataBaXe v1.3")
...
idx_label=Label(root,text='index no.').grid(row=3,column=0)
# define submit below
...
shw_notall= Button(root,text="show not all",command=shw_speci).grid(row=7,columnspan=2)
#update record-->updates pre-existing record to a new one
up_btn= Button(root,text="update record",command=updt).grid(row=8,columnspan=2)
#delete record--> deletes specific record
del_btn= Button(root,text="delete record",command=delete).grid(row=6,columnspan=2)
#show button--> shows items stored in db
shw_btn= Button(root,text="show all",command=shw_all).grid(row=5,columnspan=2)
#submit button--> making button adds command to it--> define command above
sub_butt= Button(root,text="submit new",command=submit).grid(row=4,columnspan=2)
