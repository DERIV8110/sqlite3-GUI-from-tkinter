...
def Login():
        ...
        if user.get()==3:
              ...
              if len(user_fetch)==0:
                      ...
                      my_tab2.grid(row=0,column=0,rowspan=8,columnspan=6,sticky="nsew")
                      ...
                      def show():
                                ...
                                conn.close()
                                # FOR PATIENT DB
                                tree = ttk.Treeview(my_frame11,columns=col_names,show="headings",bootstyle=DARK)
                                ...
                                # FOR STAFF DB
                                tree = ttk.Treeview(my_frame21,columns=col_names2,show="headings",bootstyle=DARK)
                      ...
                      #          c.execute("DELETE FROM OPD2 WHERE id=?",(id_no.get(),))   # DONT FORGET THE COMMA--> MAKE A TUPLE
                      #   
                      #          conn.commit()
                      #          conn.close()
        ...
        if user.get()==1:
              ...
              if len(user_fetch)==0:
                      ...
                      background_label2 = ttk.Label(my_frame1, image=my_img2)
                      background_label2.image = my_img2   # <-- attach reference to the widget
                      background_label2.place(x=0, y=0)

                      background_label2 = ttk.Label(my_frame2, image=my_img2)
                      background_label2.image = my_img2   # <-- attach reference to the widget
                      background_label2.place(x=0, y=0)

                      background_label2 = ttk.Label(my_frame5, image=my_img2)
                      background_label2.image = my_img2   # <-- attach reference to the widget
                      background_label2.place(x=0, y=0)
                      ...
                      #dashboard_button    = ttk.Button(my_frame1,text="Dashboard"  ,bootstyle=PRIMARY,command=show_dash) 
                      ...
                      def open_file():
                              ...
                              #def convert_to_doc(convert_doc):
                              #        with io.BytesIO(convert_doc) as file_like:
                              #                converted2 = Image.open(file_like).convert("RGBA")
                              #        return converted2
                              #converted_doc = convert_to_doc(convert_doc)
                              #img_reference2 = ImageTk.PhotoImage(converted_doc)
                              #doc_place = ttk.Label(pat_dash,image=img_reference2)
                              #doc_place.image = img_reference2
                              #doc_place.grid(row=0,column=2)
                      ...
                      #history_label       = ttk.Label(my_frame2,text=doc_out[0][3],font=("Segoe UI", 12, "bold"),style="dashdatalabels.TLabel").grid(row=2,column=4)      
                                      
                                      
                      #document_button  = ttk.Button(my_frame2,text="Medi. History",bootstyle=PRIMARY,command=show_docs).grid(column=0,row=2,ipadx=7) 
                      #appointment_button  = ttk.Button(pat_dash,text="Appointment",bootstyle=PRIMARY).grid(column=0,row=3)
                      ...
                      def verify_user():
                              ...
                              #for user in usernames[0]:
                              #        if 
                      aptt_user_verify = ttk.Button(my_frame3,text="Verify",command=verify_user)
                      ...
                      menu.add_command(label="Orthopedics",  command=lambda: print("Orthopedics clicked"))
                      menu.add_command(label="Opthalmology", command=lambda: print("Opthalmology clicked"))
                      menu.add_command(label="General Medicine", command=lambda: print("General Medicine"))
                      menu.add_command(label="Gastroenterology", command=lambda: print("Gastroenterology"))
                      dept_menu.config(menu=menu)
                      book_butt = ttk.Button(my_frame3,text="Book").grid(row=4,column=1)
                      #COMMAND TO BE ADDED IN book_butt TO ADD APPOINTMENTS WITH PAT_DATA INTO APTT TABLE
                      #select_doctor_label = ttk.Label(my_frame3,text="Choose Date").grid(row=2,column=1)


                      def logout():
                              pat_dash.destroy()
                      logout_label =ttk.Label(my_frame5,      text="Log off your Account>",font=("Segoe UI", 24),style="dashdatalabels.TLabel").grid(row=3,column=1,columnspan=3) 
                      logout_button = ttk.Button(my_frame5,text="LOG OUT",bootstyle=DANGER,command=logout).grid(column=5,row=3,ipadx=7)
              else:
                      messagebox.showerror("Login attempt","Invalid Username or Password!")
