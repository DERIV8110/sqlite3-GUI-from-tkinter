...
def sign_up():
        ...
        def submit():
               ...
               if user_check is None:
                      ...
                      def convert_to_binary(photo_store):
                             with open(photo_store,"rb") as file:
                                    return file.read()
                             global data
                             data = convert_to_binary(photo_store)
                      conn= sqlite3.connect('OPD2.db')
                      c=conn.cursor()
                      c.execute("""INSERT INTO OPD2(Photo,Name,Username,Password,Age,Gender,Address,Phone,Email,Medical,Doctor,Department,Diagnosis) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                 (data,name_entry.get(),username_entry.get(),password_entry.get(),age_entry.get(),gender_entry.get(),address_entry.get(),phone_entry.get(),email_entry.get(),NONE,NONE,NONE,NONE))
        ...
        photo_store = filedialog.askopenfilename(initialdir="C:/python images",title="Upload Photo",filetypes=(("png files","*.png"),("jpg files","*.jpg"),("all files","*.*")))
        ...
        #photo_upload_button = ttk.Button(signup_win,                        text="Upload",bootstyle=PRIMARY,command=convert_to_binary).grid(row=1,column=3)   # <-- GIVE COMMAND AND DEF HERE
        ...
        if user.get()==1:
              ...
              if len(user_fetch)==0:
                      ...
                      def show_db():
                                ...
                                #photo2_label = ttk.Label(pat_dash,     text=entries[0],font=("Segoe UI", 18)).grid(row=0,column=0)
